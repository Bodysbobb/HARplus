...
In this vignette, I aim to highlight the usage of grouping-data features `group_by_dims` and `group_by_dims_from_sl4` that may be useful for those GEMPACK users. Note that I firstly build this package based on the GTAPv7 models using the example of simulation solution file `<sl4>` and the Welfare decompostion file `<har>`. That's, I will use the course 3x3 simulation for this vignette. \footnote{For GTAP model information, please }
```{r load-package}
